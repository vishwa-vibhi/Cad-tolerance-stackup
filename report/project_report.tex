...
\textbf{Table Detection:} Smock et al. \cite{smock2024} tackled table structure recognition in documents using grid-based similarity metrics. Our BOM table reconstruction borrows similar ideas---spatial clustering and column boundary detection---but adapts them for the particular layout of engineering drawing Bills of Materials.

\textbf{Symbol Recognition:} Elyan et al. \cite{elyan2024} reviewed current approaches and future directions for symbol detection in engineering drawings. Their work on YOLO-based detection of engineering symbols informed our decision to train a YOLOv8 model for GD\&T symbol recognition.

\textbf{Dimension Recognition:} Ren et al. \cite{ren2024} proposed a transformer-based approach for automatic dimension extraction from mechanical drawings. Their method combines OCR with spatial reasoning to link dimensions to features. Our work takes a similar direction but uses a lighter-weight pipeline (SVM instead of transformers) that runs without cloud infrastructure.
...
\textbf{Real-time Object Detection:} Jocher et al. \cite{jocher2024yolov8} released the latest YOLO iteration for real-time detection. We use the nano variant trained on a custom Roboflow dataset of 101 annotated drawings for GD\&T symbol detection.

\textbf{Document Binarization:} Sauvola and Pietik{\"a}inen \cite{sauvola2000} compared various adaptive thresholding approaches. Their findings on handling variable illumination guided our choice of Gaussian adaptive thresholding with tuned parameters.
...
\textbf{Multi-stage CAD Analysis:} Zhao et al. \cite{zhao2024} recently proposed a multi-stage pipeline for CAD drawing analysis using vision-language models. While their approach requires significantly more computational resources, it validates the multi-stage architecture pattern that our system also follows.
...
\bibitem{moreno2024}
C. F. Moreno-Garcia, E. Elyan, and C. Jayne, ``Deep learning approaches for engineering drawing digitisation: A comprehensive review,'' \textit{Engineering Applications of Artificial Intelligence}, vol. 133, 2024, Art. no. 108521.

\bibitem{wang2024hough}
Z. Wang, L. Chen, and H. Zhang, ``Enhanced line segment detection using learnable Hough Transform for industrial applications,'' \textit{Pattern Recognition}, vol. 148, 2024, Art. no. 110152.

\bibitem{lu2024}
T. Lu, P. Wang, and H. Cai, ``Intelligent interpretation of engineering drawings: A survey of methods and applications,'' \textit{Computers in Industry}, vol. 155, 2024, Art. no. 104062.
...
\bibitem{smock2024}
B. Smock, R. Pesala, and R. Abraham, ``GriTS: Grid Table Similarity Metric for Table Structure Recognition,'' in \textit{Proc. ICDAR}, 2024, pp. 535--549.

\bibitem{elyan2024}
E. Elyan, C. F. Moreno-Garcia, and P. Johnston, ``Symbols in engineering drawings: current approaches and future opportunities,'' \textit{Artificial Intelligence Review}, vol. 57, no. 3, 2024, Art. no. 89.

\bibitem{ren2024}
S. Ren, K. Chen, and W. He, ``Automated dimension extraction from mechanical drawings using transformer-based OCR and spatial reasoning,'' \textit{Journal of Manufacturing Systems}, vol. 72, pp. 412--425, 2024.
...
\bibitem{zhao2024}
X. Zhao, Y. Liu, and J. Wang, ``A multi-stage pipeline for automated CAD drawing analysis using vision-language models,'' in \textit{Proc. IEEE Int. Conf. Industrial Informatics (INDIN)}, 2024, pp. 1--8.
